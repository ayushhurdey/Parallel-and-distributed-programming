```py
'''
(2.75 p) Write a distributed program, using MPI, that computes all prime numbers up to N. The first shall distribute to all others the list of primes up to √N, then each of the processes shall take its fair share of the interval √N to N, check all the numbers there, and send the primes to process 0.
'''
