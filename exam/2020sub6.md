...
```py
'''
(2.75 p) Write a parallel program, using at most a given number of threads, that finds a permutation of N that satisfies a given property, if such permutation exists, or an empty vector if no permutation satisfy the given property.
    You are given a function `bool pred(vector<int> const& v) that verifies if a given permutation satisfies the property; your program should call the function for each possible permutation (until it finds a solution).
    At each level in the search tree, each branch shall be explored by a different thread if enough threads are available.
'''
